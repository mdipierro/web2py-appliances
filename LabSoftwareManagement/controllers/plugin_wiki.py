...
def comment():
    ...
    comments=db(table.tablename==tablename)(table.record_id==record_id).select()
...
def cloud():
    ...
    return DIV(_class='plugin_wiki_tag_cloud',*[SPAN(A(tag.name,_href=URL('plugin_wiki','page',args=('tag',tag.id))),_style='font-size:%sem' % (0.8+1.0*tag.links/mc)) for tag in tags])
