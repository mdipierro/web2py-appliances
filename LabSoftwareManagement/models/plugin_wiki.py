...
""" % dict(callback=callback,colnames=json(colnames),
    ...
    def comments(table='None',record_id=None):
        ...
        """
        return LOAD('plugin_wiki','comment',user_signature=True,
    ...
    def tags(table='None',record_id=None):
        ...
        return LOAD('plugin_wiki','tags',user_signature=True,
                    args=(table,record_id or 0),ajax=True)
    ...
    @staticmethod
    def load_action(action, controller='', ajax=True, user_signature=True):
        """
        loads the content of URL(request.application, controller, action) via ajax
        and traps forms
        """
        return LOAD(controller, action, ajax=ajax, user_signature=user_signature)
