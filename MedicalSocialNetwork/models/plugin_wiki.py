...
if not 'plugin_wiki_mode' in globals():
    plugin_wiki_mode = 'markmin'

if not 'plugin_wiki_level' in globals():
    plugin_wiki_level = 3 # 1 - wiki only, 2 - widgets enables, 3 - remplate render enabled

if not 'plugin_wiki_migrate' in globals():
    plugin_wiki_migrate = True


###################################################
# js and css modules required by the plugin
###################################################
for _f in ['plugin_wiki/ui/ui-darkness/jquery-ui-1.8.1.custom.css',
...
""" % dict(callback=callback,colnames=json(colnames),
    ...
    def pie_chart(data,names,width=300,height=150,align='center'):
        ...
        """
        data = data
        names = '|'.join(x.strip() for x in names.split(','))
...
class PluginWiki:
    ...
    class widget_builder:
        ...
        def xml(self):
            return '<div id="%(id)s" style="display:none"><div style="position:fixed;top:0%%;left:0%%;width:100%%;height:100%%;background-color:black;z-index:1001;-moz-opacity:0.8;opacity:.80;opacity:0.8;"></div><div style="position:fixed;top:%(top)s%%;left:%(left)s%%;width:%(width)s%%;height:%(height)s%%;padding:16px;border:2px solid black;background-color:white;opacity:1.0;z-index:1002;overflow:auto;-moz-border-radius: 10px; -webkit-border-radius: 10px;"><span style="font-weight:bold">%(title)s</span><span style="float:right">[<a href="#" onclick="jQuery(\'#%(id)s\').hide();return false;">%(close)s</a>]</span><hr/><div style="width:100%%;height:90%%;" id="c%(id)s"><iframe id="widget_builder_modal_content" style="width:100%%;height:100%%;border:0">loading...</iframe></div></div></div><a href="#" onclick="jQuery(\'#widget_builder_modal_content\').attr(\'src\',\'%(source)s\');jQuery(\'#%(id)s\').fadeIn(); return false" id="plugin_wiki_open_attachments"">%(title)s</a>' % dict(title=self.caption,source=self.source,close=self.close,id=self.id,left=(100-self.width)/2,top=(100-self.height)/2,width=self.width,height=self.height)
